<template>
  <div>
    <ComponentSection>
      <m-icon-button
        v-model="toggleData"
        :ripple="checkboxProps[0].value"
        :unbounded="checkboxProps[1].value"
        :class="classes"
      >
        <svg
          v-if="radioProps[0].value"
        ...
        <template v-if="radioProps[2].value">
          favorite
        </template>
        <m-icon
          v-if="radioProps[3].value"
          slot="toggleOn"
          icon="favorite"
        />
        <m-icon
...
export default {
  data () {
    return {
      toggleData: true,
      radioProps: [
        { prop: 'basic (svg)', value: false },
        { prop: 'toggle (svg)', value: true },
  ...
  computed: {
    classes () {
      return {
        'material-icons': this.radioProps[2].value
      }
    }
  }
}
